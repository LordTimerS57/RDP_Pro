...
import { getRouteSegments, hasKeys, hasMissingPathParams, interpolatePath, rootRouteId, trimPath } from "@tanstack/router-core";
...
function RouteComp({ routerState, pendingMatches, router, route, isRoot, activeId, setActiveId }) {
	...
	const navigationTarget = createMemo(() => {
		...
		const pathname = segments ? interpolatePath(route.fullPath, segments, allParams, router().pathParamsDecoder) : route.fullPath;
		return segments && hasMissingPathParams(segments, allParams) ? void 0 : pathname;
	});
	return (() => {
...
//# sourceMappingURL=BaseTanStackRouterDevtoolsPanel-B-iktbLX.js.map