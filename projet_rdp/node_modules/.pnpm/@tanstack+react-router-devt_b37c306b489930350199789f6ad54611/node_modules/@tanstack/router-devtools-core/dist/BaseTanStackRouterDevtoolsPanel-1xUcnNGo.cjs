...
function RouteComp({ routerState, pendingMatches, router, route, isRoot, activeId, setActiveId }) {
	...
	const navigationTarget = require_context.createMemo(() => {
		...
		const segments = (0, _tanstack_router_core.getRouteSegments)(route);
		const pathname = segments ? (0, _tanstack_router_core.interpolatePath)(route.fullPath, segments, allParams, router().pathParamsDecoder) : route.fullPath;
		return segments && (0, _tanstack_router_core.hasMissingPathParams)(segments, allParams) ? void 0 : pathname;
	});
	return (() => {
...
//# sourceMappingURL=BaseTanStackRouterDevtoolsPanel-1xUcnNGo.cjs.map